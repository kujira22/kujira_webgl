...
window.onload = function() {
    ...
    //console.log("Is SplitText registered?", gsap.plugins.includes(SplitText));

    console.log(" Fancy split text")

    let mySplitText = new gsap_SplitText__WEBPACK_IMPORTED_MODULE_1__.SplitText('.intro_quote_body', { type: 'lines' });
    let lines = mySplitText.lines; // Array of each line

    // Position each line absolutely within the container
    lines.forEach((line, index) => {
        line.style.position = 'absolute';
        line.style.top = (index * 100) + '%'; // Adjust as needed
        line.style.left = 0;
    });

    let animated = false;
    window.addEventListener('scroll', function() {
        if (isElementInView('.intro_quote_body', 100) && !animated) {
            animateLines(lines);
            animated = true;
        }
    });
...
function isElementInView(selector, offset = 0) {
...
{ y: '100%' },
{ y: '0%', duration: 1, ease: 'power2.out', delay: index * 0.5 }
);
});
