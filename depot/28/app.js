...
function playBackgroundVideo() {
  ...
  // If a video is found, attempt to play it
  if (video) {
...
function insertWorkVideos() {
    ...
    // Function to handle the insertion of video into specified elements
    function insertVideo(positionElementId, videoDataElementId, targetClassPrefix) {
        ...
        var position = positionElement ? positionElement.textContent.trim() : '1'; // Default to first item if not found
        var videoURL = videoURLElement ? videoURLElement.textContent.trim() : '';

        console.log(videoURL);

        if (!videoURL) {
            console.error('Video URL not found for ' + videoDataElementId);
        ...
        // Use the position to find the target element and inject the video
        var target = document.querySelector('.' + targetClassPrefix + ':nth-child(' + position + ') .bg-vid-detail');
        if (target) {
            target.innerHTML = videoEmbedHTML;
            target.style.display = 'block'; // Ensure the video embed is visible
        }
    }

    // Insert the first instance of the video
    insertVideo('#vid-position-1', '#vid-data-1', 'work-detail-item');

    // Insert the second instance of the video
    insertVideo('#vid-position-2', '#vid-data-2', 'work-detail-item2'); // Adjust class prefix as needed

    // Insert the third instance of the video
    insertVideo('#vid-position-3', '#vid-data-3', 'work-detail-item2'); // Adjust class prefix and detail class as needed
}
