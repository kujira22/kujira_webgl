...
function insertWorkVideos() {
    ...
    // Use querySelector to find the elements by ID and get their text content
    var positionElement = document.querySelector('#vid-position-1');
    var videoURLElement = document.querySelector('#vid-data-1');

    

    // Check if elements exist and get their content, otherwise set defaults or handle error
    ...
    // If there's no video URL, you might want to handle this case differently
    if (!videoURL) {
        console.error('Video URL not found.');
        return; // Exit the function if there's no video URL
    }
    ...
    // Use the position to find the target .bg-vid-detail element and inject the video
    var target = document.querySelector('.work-detail-item:nth-child(' + position + ') .bg-vid-detail');
    if (target) {
