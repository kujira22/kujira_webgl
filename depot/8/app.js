...
function imageParallax(container) {
  container.querySelectorAll('.image-p').forEach(img => {
    ...
    function adjustMaskAndInitParallax() {
      ...
      const maskHeight = img.offsetHeight * 0.96; // Assuming img.offsetHeight is the full height
  ...
  function initParallax(img, maskHeight) {
    const movementDistance = maskHeight * 0.04; // 4% of the mask height

    gsap.fromTo(img, {
      y: 0,
    }, {
      y: `${movementDistance}px`, // Move down by 4% of the mask height
      ease: "none",
      scrollTrigger: {
