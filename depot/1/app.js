...
function imageParallax(container) {  
  const movementDistancePercent = 8;
  ...
  container.querySelectorAll('.image-p').forEach(img => {
      function setMaskHeight() {
          ...
          const maskWidth = img.parentElement.offsetWidth;
          // Calculate the target height as 8% smaller than the image's height
          const targetHeightPercent = 100 - movementDistancePercent; // Make the container 8% smaller
          const targetHeightPx = maskWidth * aspectRatio * (targetHeightPercent / 100);
          img.parentElement.style.height = `${targetHeightPx}px`;
      ...
      if (img.complete) {
          ...
          img.onload = setMaskHeight;
      }
  });

  function initParallax(img) {
      // Convert the movement distance from percentage to pixels
      const movementDistancePx = img.naturalHeight * (movementDistancePercent / 100);

      gsap.fromTo(img, {
          y: 0,
      }, {
          y: `${movementDistancePx}px`, // Move the image by calculated pixels
          ease: "none",
          scrollTrigger: {
...
function pageIntro(container) {
 ...
 gsap.set(container.querySelectorAll('.intro-reveal'), { opacity: 1 });
    ...
    pageTitles.forEach(title => {
        // Wrap each text node and preserve span styling and spaces
...
if (child.nodeType === 3 && child.textContent.trim()) { // If it's a text node with non-whitespace content
...
} else if (child.nodeType === 1) { // If it's an element
