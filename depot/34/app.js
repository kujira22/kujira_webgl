...
function playBackgroundVideo() {
  ...
  const video = document.querySelector('.w-background-video-atom video');

  console.log("trying to replay home vid");
  if (video) {
...
function insertWorkVideos() {
  console.log("running work video inserter");

  function insertVideo(positionElementId, videoDataElementId, targetClassPrefix) {
      var positionElement = document.querySelector(positionElementId);
      var videoURLElement = document.querySelector(videoDataElementId);

      if (!videoURLElement || !positionElement) {
          console.log('Either position element or video data element not found for ', positionElementId, videoDataElementId);
          return; // Exit if elements are missing
      }

      var position = positionElement.textContent.trim();
      var videoURL = videoURLElement.textContent.trim();

      if (!videoURL) {
          console.log('Video URL not found for', videoDataElementId);
          return; // Exit if there's no video URL
      }
      ...
      var target = document.querySelector('.' + targetClassPrefix + ':nth-child(' + position + ') .bg-vid-detail');
      if (target) {
          target.innerHTML = videoEmbedHTML;
          target.style.display = 'block';
      }
  }

  // Calls for each video instance
  insertVideo('#vid-position-1', '#vid-data-1', 'work-detail-item');
  insertVideo('#vid-position-2', '#vid-data-2', 'work-detail-item2');
  insertVideo('#vid-position-3', '#vid-data-3', 'work-detail-item2');
}
...
afterEnter(data) {
         ...
         if (data.next.container.querySelector('#expertise-page')) {
            $('.w-dyn-empty').parents('.awards-year-block').each(function(){ $(this).hide()})
           console.log('removed empty years');
          }
        ...
        /*
          if (data.next.container.querySelector('#home-page')) {
            setTimeout(() => {
                const homeintroTrigger = document.querySelector('.trigger-home-return');
                if (homeintroTrigger) {
                    homeintroTrigger.click();
                    console.log('.trigger home text intro after 0ms');
                }
            }, 0);
        }
        ...
        */
        ...
        if (data.next.container.querySelector('#work-detail')) {
            modalsetup();  
        ...
        if (data.next.container.querySelector('#work-filters')) {
            filterScript('https://cdn.jsdelivr.net/npm/@finsweet/attributes-cmsfilter@1/cmsfilter.js', function() {
                console.log('work filter script loaded');
              });
          }

          if (data.next.container.querySelector('#news-filters')) {
            filterScript('https://cdn.jsdelivr.net/npm/@finsweet/attributes-cmsfilter@1/cmsfilter.js', function() {
                    console.log('news filter script loaded');
                  });
          }
