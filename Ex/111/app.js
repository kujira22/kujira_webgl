...
function setupSlider(comboClass, p_num) {
    ...
    function animateCoverImage(isEntering) {
        // Only apply animations if the screen width is 475px or less
        if (window.innerWidth <= 475) {
            let slideWidth = getMoveDistance();
            let currentSlideIndex = Math.abs(currentTranslationX / slideWidth);

            let currentSlides = document.querySelectorAll(`.slider_content.${comboClass} .panel_item:nth-child(${currentSlideIndex + 1}) .bg_km.mob, .slider_content.${comboClass} .panel_item:nth-child(${currentSlideIndex + 1}) .bg_km.bg_p.mob`);
            ...
            currentSlides.forEach(slide => {
                if (isEntering) {
                    ...
                    let previousSlides = document.querySelectorAll(`.slider_content.${comboClass} .panel_item:nth-child(${currentSlideIndex}) .bg_km.mob, .slider_content.${comboClass} .panel_item:nth-child(${currentSlideIndex}) .bg_km.bg_p.mob`);
