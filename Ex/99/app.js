...
const introsound = new howler__WEBPACK_IMPORTED_MODULE_1__.Howl({
...
function menuchaos() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.menu_burger').forEach(button => {
            ...
            document.querySelectorAll('.sb01, .sb02, .sb03').forEach(element => {
                element.addEventListener('click', function() {
                    lenis.start();
                });
            });
...
function initAOSTextAnimation(useAOS = true) {
    ...
    if (useAOS) {
        ...
        const nonSplitTextElements = document.querySelectorAll('.pagewrapper *:not(p):not(.filter):not(.bg_k):not(.submit_btn):not(.bg_km):not(.bg_k_slider):not(.btn_hovered_inner):not(.bg_s):not(.bg_w):not(.field_dropdown):not(.bg_b):not(.paragraph.medium.towhite)');
