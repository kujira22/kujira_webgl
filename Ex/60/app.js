...
document.addEventListener('click', function(event) {
  if (event.target.tagName.toLowerCase() === 'a' && event.target.hash) {
    event.preventDefault();
    let targetUrl = event.target.href.split('#')[0];
    let targetHash = event.target.hash;

    if (window.location.href === targetUrl || targetUrl === '') {
      scrollToAnchor(targetHash);
    } else {
      _barba_core__WEBPACK_IMPORTED_MODULE_5___default().go(event.target.href);
      sessionStorage.setItem('postLoadAnchor', targetHash);
    }
  }
});
    ...
    transitions: [
        {
            ...
            let postLoadAnchor = sessionStorage.getItem('postLoadAnchor');
            if (postLoadAnchor) {
              sessionStorage.removeItem('postLoadAnchor');
              scrollToAnchor(postLoadAnchor);
            }
