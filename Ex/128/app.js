...
/////////// Colour switching chaos©

// Function to apply the saved mode on page load or transition
function applySavedMode() {
  ...
  document.querySelectorAll('.light-a').forEach(el => el.style.display = mode === 'dark' ? 'none' : 'block');
  document.querySelectorAll('.dark-a').forEach(el => el.style.display = mode === 'dark' ? 'block' : 'none');
  console.log(`Applied ${mode} mode on page load.`);
}

// Function to toggle the mode, save it, and reload the page
function toggleMode() {
  const newMode = localStorage.getItem('mode') === 'dark' ? 'light' : 'dark';
  localStorage.setItem('mode', newMode);
  console.log(`Toggled to ${newMode} mode. Reloading page...`);
  location.reload();
}


// Initialization function to apply mode and set up event listeners
function initializeMode() {
  applySavedMode(); // Apply the saved mode on load or transition
  document.getElementById('mode').addEventListener('click', toggleMode); // Set up the toggle event listener
}


function runAnimationBasedOnMode() {
  // Get the current mode from localStorage or default to 'light'
  const mode = localStorage.getItem('mode') || 'light';
  console.log(`Current mode: ${mode}`);

  // Run the appropriate animation function based on the mode
  if (mode === 'dark') {
      console.log("Running dark mode animation");
      initAOSTextAnimation();
  } else {
      console.log("Running light mode animation");
      initAOSTextAnimationLight();
  }
}
    ...
    transitions: [
        {
            ...
            enter({ current, next, trigger }) {  
              ...
              const isDarkPage = next.container.querySelector('.darkdiv') !== null;
              const isLightPage = next.container.querySelector('.lightdiv') !== null;
          
              if (trigger && (trigger.classList.contains('menu_link') || trigger.classList.contains('home_link_menu') ||  trigger.classList.contains('menu_filter') ||  trigger.classList.contains('menu_link_large'))) {
                  console.log("triggered by menu link");
          
                  if (isDarkPage) {
                      console.log("darkdiv found");
                  ...
                  // Triggered by other links
                  transIn();
                  console.log("triggered by normal link");
...
function menuchaos() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const fgColors = ['#6003e5', '#6003e5', '#6D0133', '#FF00FF'];
        const bgColors = ['#FF00FF', '#6A5101', '#FF00FF', '#FF6A00'];
...
function initAOSTextAnimation(useAOS = true) {
    console.log("home AOS text chaos running (dark page)")
...
console.log("internal Aos running (light page)")
    ...
    const startingColor = '#FF00FF';
    const targetColor = '#000000';

    // Custom colors for .towhite
    const toWhiteStartingColor = '#6BFE02';  
    const toWhiteTargetColor = '#F2F2ED';   
    ...
    nonSplitTextElements.forEach(element => {
     ...
     fields.forEach(field => {
        const overlay_white = field.nextElementSibling;
         ...
         field.addEventListener('focus', () => {
             gsap__WEBPACK_IMPORTED_MODULE_0__.gsap.to(field, {
               color: field.classList.contains('select_field') ? startingColor : '#F2F2ED',
               duration: 0.3,
             });
...
function setupSlider(comboClass, p_num) {
    ...
    function animateSlider(x, reset = false) {
        let elementsToAnimate = document.querySelectorAll(`.slider_content.${comboClass}, .slider_content.${comboClass}.b`);
        if (elementsToAnimate.length > 0) {
            if(reset) {
                gsap__WEBPACK_IMPORTED_MODULE_0__.gsap.to(elementsToAnimate, {
                    duration: 0.3,
                    x: `${x}vw`,
                    ease: "expo.inOut",
    ...
    function resetSlider() {
        let elementsToReset = document.querySelectorAll(`.slider_content.${comboClass}, .slider_content.${comboClass}.b`);
        if (elementsToReset.length > 0) {
            gsap__WEBPACK_IMPORTED_MODULE_0__.gsap.set(elementsToReset, { x: '0vw' });
    ...
    let nextButton = document.querySelector(`.slider_next.${comboClass}`);
    if (nextButton) {
