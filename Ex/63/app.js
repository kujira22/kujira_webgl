...
function isOnHomePage() {
  return window.location.pathname === '/' || window.location.pathname === '/index.html'; // Adjust this based on your site's structure
}
...
_barba_core__WEBPACK_IMPORTED_MODULE_5___default().init({
    ...
    before: (data) => {
      if (isOnHomePage() && data.next.url.path === '/') { 
        // If we're already on the homepage and the clicked link is also pointing to the homepage
        if (data.trigger && data.trigger.id === 'contact_link') {
          document.getElementById('contact').scrollIntoView({ behavior: 'smooth' });
          return false; // Stop the transition
        }
      }
    },
